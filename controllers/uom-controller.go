...
func UomSave(c *fiber.Ctx) error {
	...
	resp, err := axios.R().
		...
		SetBody(map[string]interface{}{
			...
			"page":            client.Page,
			"sdata":           client.Sdata,
			"uom_id":          client.Uom_id,
			"uom_name":        client.Uom_name,
			"uom_status":      client.Uom_status,
		}).
		Post(PATH + "api/uomsave")
	if err != nil {
		log.Println(err.Error())
	}
	log.Println("Response Info:")
