<script> 
    ...
    async function handleSave() {
        ...
        if(flag){
            ...
            msgloader = "Sending...";
            const res = await fetch("/api/categoryitemsave", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    sdata: sData,
                    page:"CURR-SAVE",
                    cateitem_search: searchHome,
                    cateitem_page: parseInt(pagingnow),
                    cateitem_id: parseInt(idrecord),
                    cateitem_name: name_field,
                    cateitem_status: status_field,
                }),
            });
                    ...
                    <table class="table table-striped ">
                        <thead>
                            <tr>
                                ...
                                <th NOWRAP width="*" style="text-align: left;vertical-align: top;font-weight:bold;font-size: {table_header_font};">CATEGORY ITEM</th>
                            </tr>
                        </thead>
                        {#if totalrecord > 0}
                        <tbody>
