<script>
    ...
    async function initHome(e) {
        listHome = [];
        const res = await fetch("/api/categoryitem", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: "Bearer " + token,
            ...
            if (record != null) {
                ...
                for (var i = 0; i < record.length; i++) {
                    ...
                    listHome = [
                        ...
                        {
                            home_no: no,
                            home_id: record[i]["cateitem_id"],
                            home_name: record[i]["cateitem_name"],
                            home_status: record[i]["cateitem_status"],
                            home_status_css: record[i]["cateitem_status_css"],
                            home_create: record[i]["cateitem_create"],
                            home_update: record[i]["cateitem_update"],
                        },
                    ];
                }
                listPage = [];
